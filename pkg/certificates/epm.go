...
func EpmDemo() {
	...
	cli, err := epm.NewEpmClient(ctx, cc, dcerpc.WithSign(), dcerpc.WithTargetName(os.Getenv("TARGET")), dcerpc.WithVerifyBitMask(true), dcerpc.WithVerifyPresentation(true), dcerpc.WithVerifyHeader2(true), dcerpc.WithLogger(zerolog.New(os.Stdout)))
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return
	}
	...
	MapSyntax(ctx, cli, icertrequestd2.CertRequestD2SyntaxV0_0)
}
