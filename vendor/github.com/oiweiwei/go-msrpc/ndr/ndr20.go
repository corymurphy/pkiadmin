...
func NDR20(buf []byte, opts ...any) NDR {
	...
	ndr.refid = new(atomic.Uint32)
	ndr.refid.Store(0x00020000) // CAMHACK
...
func (w *ndr20) WriteData(d any) error {
	...
	// if len(buf) == 4 {
	// 	if buf[0] == 0x25 && buf[1] == 0x00 && buf[2] == 0x00 && buf[3] == 0x00 {
	// 		fmt.Println("buf:", buf, d)
	// 	}
	// }
...
func (w *ndr20) WritePointer(ptr Pointer, mrs ...Marshaler) error {
	...
	// fmt.Println(w.buf)
	// fmt.Println(uint32(w.buf.Pos() + 1))
	// fmt.Println()

	// fmt.Println(0x00020000)
	// fmt.Println(0x00020000 + 4)

	// CAMHACK
	// fmt.Println("refid:", w.refid)

	if err := w.WriteData(uint32(w.refid.Load())); err != nil {
		return w.SetErr(err)
	}
